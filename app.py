...
df.rename(columns = {'phase3':'phrase3'}, inplace = True)
df = df[:10]
#df.head(10)
list_col = []
for i in df.columns:
...
y_train = np.array(list(df1.intent))
import numpy as np
...
outputs = tf.keras.layers.Dense(len(zx), activation=tf.nn.softmax)(x)
one_nn = tf.keras.Model(inputs=inputs, outputs=outputs)
one_nn.compile(loss="sparse_categorical_crossentropy",
                      optimizer="adam",
                      metrics=["sparse_categorical_accuracy"])
one_nn.fit(X_train, y_train, epochs=75, batch_size=25)
...
def entry1():
	...
	ctime = time.time()
	dpredictions = one_nn.predict(np.array(models.encode([translator.translate(name).text])))
	varx = np.argmax(dpredictions, axis=1)
	return {"data":zx[varx[0]],
